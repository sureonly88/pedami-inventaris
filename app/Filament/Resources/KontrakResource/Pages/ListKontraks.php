...
class ListKontraks extends ListRecords
{
    ...
    protected static ?string $title = 'Kontrak Sewa Kendaraan';
    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Actions\Action::make('cetak_laporan')
                ...
                ->color('success')
                ->modalHeading('Cetak Laporan Daftar Kontrak')
                ...
                ->form([
                    Grid::make(2)->schema([
                        Select::make('filter_type')
                            ->label('Tipe Filter')
                            ->options([
                                'range' => 'Rentang Tanggal (Tgl Awal)',
                                'monthly' => 'Bulan & Tahun (Tgl Awal)',
                            ])
                            ->default('range')
                            ->reactive()
                            ->required(),
                ...
                ->action(function (array $data) {
                    ...
                    if ($data['format'] === 'excel') {
                        return Excel::download(
                            new KontrakExport($filters, $period),
                            'Laporan_Daftar_Kontrak_' . str_replace(' ', '_', $period) . '.xlsx'
                        );
                    } else {
                        $records = Kontrak::with('kontrakDetails.dataR2r4')
                            ->when($filters['from'] ?? null, fn($q, $from) => $q->whereDate('tgl_awal', '>=', $from))
                            ->when($filters['until'] ?? null, fn($q, $until) => $q->whereDate('tgl_awal', '<=', $until))
                            ->when($filters['month'] ?? null, fn($q, $month) => $q->whereMonth('tgl_awal', $month))
                            ->when($filters['year'] ?? null, fn($q, $year) => $q->whereYear('tgl_awal', $year))
                            ->get();

                        $pdf = Pdf::setPaper('a4', 'portrait')->loadHtml(
                            Blade::render('filament.reports.kontrak-report', [
                                'records' => $records,
                                'period' => $period
                            ])
                        );
                        ...
                        return response()->streamDownload(function () use ($pdf) {
                            echo $pdf->stream();
                        }, 'Laporan_Daftar_Kontrak_' . str_replace(' ', '_', $period) . '.pdf');
                    }
                }),
        ];
    }
