...
use App\Exports\MutasiAssetExport;
    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Actions\Action::make('cetak_laporan')
                ...
                ->color('success')
                ->modalHeading('Cetak Laporan Mutasi Aset')
                ...
                ->action(function (array $data) {
                    ...
                    if ($data['format'] === 'excel') {
                        return Excel::download(
                            new MutasiAssetExport($filters, $period),
                            'Laporan_Mutasi_' . str_replace(' ', '_', $period) . '.xlsx'
                        );
                    } else {
                        $records = MutasiAsset::with(['asset', 'ruangan_a', 'penanggung_jawab_a', 'karyawan_a', 'ruangan_t', 'penanggung_jawab_t', 'karyawan_t'])
                        ...
                        $pdf = Pdf::setPaper('a4', 'landscape')->loadHtml(
                            Blade::render('filament.reports.mutasi-report', [
                                'records' => $records,
                                'period' => $period
                            ])
                        );
                        ...
                        return response()->streamDownload(function () use ($pdf) {
                            echo $pdf->stream();
                        }, 'Laporan_Mutasi_' . str_replace(' ', '_', $period) . '.pdf');
                    }
                }),
        ];
    }
