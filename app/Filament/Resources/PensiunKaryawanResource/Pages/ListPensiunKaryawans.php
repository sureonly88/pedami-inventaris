...
use App\Exports\PensiunKaryawanExport;
...
class ListPensiunKaryawans extends ListRecords
{
    protected static string $resource = PensiunKaryawanResource::class;
    protected static ?string $title = 'Pensiun Karyawan';
    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Actions\Action::make('cetak_laporan')
                ...
                ->color('success')
                ->modalHeading('Cetak Laporan Pensiun Karyawan')
                ...
                ->action(function (array $data) {
                    ...
                    if ($data['format'] === 'excel') {
                        return Excel::download(
                            new PensiunKaryawanExport($filters, $period),
                            'Laporan_Pensiun_Karyawan_' . str_replace(' ', '_', $period) . '.xlsx'
                        );
                    } else {
                        $records = PensiunKaryawan::with(['karyawan', 'divisiTerakhir', 'subdivisiTerakhir'])
                            ->when($filters['from'] ?? null, fn ($q, $from) => $q->whereDate('tgl_pensiun', '>=', $from))
                            ->when($filters['until'] ?? null, fn ($q, $until) => $q->whereDate('tgl_pensiun', '<=', $until))
                            ->when($filters['month'] ?? null, fn ($q, $month) => $q->whereMonth('tgl_pensiun', $month))
                            ->when($filters['year'] ?? null, fn ($q, $year) => $q->whereYear('tgl_pensiun', $year))
                            ->get();

                        $pdf = Pdf::setPaper('a4', 'landscape')->loadHtml(
                            Blade::render('filament.reports.pensiun-karyawan-report', [
                                'records' => $records,
                                'period' => $period
                            ])
                        );
                        ...
                        return response()->streamDownload(function () use ($pdf) {
                            echo $pdf->stream();
                        }, 'Laporan_Pensiun_Karyawan_' . str_replace(' ', '_', $period) . '.pdf');
                    }
                }),
        ];
    }
