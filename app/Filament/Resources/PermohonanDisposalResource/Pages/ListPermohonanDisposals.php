...
use App\Exports\PermohonanDisposalExport;
    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Actions\Action::make('cetak_laporan')
                ...
                ->color('success')
                ->modalHeading('Cetak Laporan Disposal Aset')
                ...
                ->action(function (array $data) {
                    ...
                    if ($data['format'] === 'excel') {
                        return Excel::download(
                            new PermohonanDisposalExport($filters, $period),
                        ...
                        $records = PermohonanDisposal::with(['asset.ruangan', 'dibuatOleh'])
                            ...
                            ->when($filters['year'] ?? null, fn ($q, $year) => $q->whereYear('tgl_pengajuan', $year))
                            ->get();

                        $pdf = Pdf::setPaper('a4', 'landscape')->loadHtml(
                            Blade::render('filament.reports.disposal-report', [
                                'records' => $records,
                                'period' => $period
                            ])
                        );
                        ...
                        return response()->streamDownload(function () use ($pdf) {
                            echo $pdf->stream();
                        }, 'Laporan_Disposal_' . str_replace(' ', '_', $period) . '.pdf');
                    }
                }),
        ];
    }
