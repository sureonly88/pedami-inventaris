...
class ListMutasiKaryawans extends ListRecords
{
    protected static string $resource = MutasiKaryawanResource::class;
    protected static ?string $title = 'Mutasi Karyawan';
    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Actions\Action::make('cetak_laporan')
                ...
                ->color('success')
                ->modalHeading('Cetak Laporan Mutasi Karyawan')
                ...
                ->action(function (array $data) {
                    ...
                    if ($data['format'] === 'excel') {
                        return Excel::download(
                            new MutasiKaryawanExport($filters, $period),
                            'Laporan_Mutasi_Karyawan_' . str_replace(' ', '_', $period) . '.xlsx'
                        );
                    } else {
                        $records = MutasiKaryawan::with(['karyawan', 'divisiAsal', 'subdivisiAsal', 'divisiTujuan', 'subdivisiTujuan'])
                        ...
                        $pdf = Pdf::setPaper('a4', 'landscape')->loadHtml(
                            Blade::render('filament.reports.mutasi-karyawan-report', [
                                'records' => $records,
                                'period' => $period
                            ])
                        );
                        ...
                        return response()->streamDownload(function () use ($pdf) {
                            echo $pdf->stream();
                        }, 'Laporan_Mutasi_Karyawan_' . str_replace(' ', '_', $period) . '.pdf');
                    }
                }),
        ];
    }
