...
class ListPenjualanR2r4s extends ListRecords
{
    ...
    protected static ?string $title = 'Penjualan Kendaraan';
                ...
                ->action(function (array $data) {
                    ...
                    if ($data['format'] === 'excel') {
                        return Excel::download(
                            new PenjualanDirectExport(null, $filters, $period),
                            'Laporan_Penjualan_' . str_replace(' ', '_', $period) . '.xlsx'
                        );
                    } else {
                        $records = PenjualanR2r4::with('data_r2r4')
                            ->when($filters['from'] ?? null, fn($q, $from) => $q->whereDate('tgl_jual', '>=', $from))
                            ->when($filters['until'] ?? null, fn($q, $until) => $q->whereDate('tgl_jual', '<=', $until))
                            ->when($filters['month'] ?? null, fn($q, $month) => $q->whereMonth('tgl_jual', $month))
                            ->when($filters['year'] ?? null, fn($q, $year) => $q->whereYear('tgl_jual', $year))
                            ->get();

                        $pdf = Pdf::loadHtml(
                            Blade::render('filament.reports.penjualan-report', [
                                'records' => $records,
                                'period' => $period
                            ])
                        );
                        ...
                        return response()->streamDownload(function () use ($pdf) {
                            echo $pdf->stream();
                        }, 'Laporan_Penjualan_' . str_replace(' ', '_', $period) . '.pdf');
                    }
                }),
        ];
    }
